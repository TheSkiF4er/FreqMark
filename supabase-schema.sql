...
create table if not exists public.legend_models (
  ...
  status text not null default 'ready' check (status in ('ready', 'destroyed', 'lost')),
...
alter table public.legend_models add column if not exists status text;
update public.legend_models
set status = coalesce(nullif(status, ''), 'ready')
where status is distinct from coalesce(nullif(status, ''), 'ready');
alter table public.legend_models alter column status set default 'ready';
alter table public.legend_models alter column status set not null;
alter table public.legend_models drop constraint if exists legend_models_status_check;
alter table public.legend_models
  add constraint legend_models_status_check check (status in ('ready', 'destroyed', 'lost'));
